// src/routes/api/gallery_images/index.tsx
...
export const onGet: RequestHandler = async ({ json, env }) => {
  ...
  try {
    console.log('GET /api/gallery_images - Creating Turso client...');
    const client = await tursoClient({ env });
    console.log('GET /api/gallery_images - Client created, calling getGalleryImages...');
    ...
    console.error('GET /api/gallery_images error:', error);
...
export const onPost: RequestHandler = async ({ request, json, env }) => {
  ...
  try {
    ...
    console.log('POST /api/gallery_images - Creating Turso client...');
    const client = await tursoClient({ env });
    console.log('POST /api/gallery_images - Client created, calling createGalleryImage...');
    ...
    console.error('POST /api/gallery_images error:', error);
...
export const onPut: RequestHandler = async ({ request, json, env }) => {
  ...
  try {
    ...
    console.log('PUT /api/gallery_images - Creating Turso client...');
    const client = await tursoClient({ env });
    console.log('PUT /api/gallery_images - Client created, calling updateGalleryImage...');
    ...
    console.error('PUT /api/gallery_images error:', error);
...
export const onDelete: RequestHandler = async ({ request, json, env }) => {
  ...
  try {
    ...
    if (!id) {
      console.log('DELETE /api/gallery_images - Missing ID:', { id });
      json(400, { error: 'ID is required' });
      return;
    }

    console.log('DELETE /api/gallery_images - Creating Turso client...');
    const client = await tursoClient({ env });
    console.log('DELETE /api/gallery_images - Client created, calling deleteGalleryImage...');
    await deleteGalleryImage(client, id);
    console.log('DELETE /api/gallery_images - Image deleted with ID:', id);

    json(200, { success: true });
  } catch (error) {
    console.error('DELETE /api/gallery_images error:', error);
